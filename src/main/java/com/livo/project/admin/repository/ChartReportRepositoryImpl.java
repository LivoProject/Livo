...
""";
    ...
    SELECT
      r.lectureId                                        AS lectureId,
      COUNT(*)                                          AS total,
      SUM(CASE WHEN r.status IN ('CONFIRMED','SUCCESS') THEN 1 ELSE 0 END) AS confirmed,
      SUM(CASE WHEN r.status IN ('CANCEL','CANCELED','FAIL') THEN 1 ELSE 0 END) AS canceled,
      SUM(CASE WHEN r.status = 'PENDING' THEN 1 ELSE 0 END) AS pending,
      0 AS expired
    FROM reservation r
    WHERE r.createdAt BETWEEN :from AND :to
